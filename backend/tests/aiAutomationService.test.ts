...
import {
  ...
  rejectAction,
} from '../src/services/aiAutomationService';
...
describe('AI automation approval protection', () => {
  ...
  it('stores a transporter bid as PENDING_APPROVAL and does not submit it during preparation', async () => {
    mockPrisma.$queryRawUnsafe.mockResolvedValueOnce([
      {
        id: 'action-1',
        user_id: 'transporter-1',
        role: 'TRANSPORTER',
        action_name: 'place_bid',
        status: 'PENDING_APPROVAL',
        payload: { loadId: 'load-1', amount: 400000 },
        expires_at: new Date(Date.now() + 30 * 60 * 1000),
      },
    ]);

    const result = await processAutomationMessage(
      'transporter-1',
      'TRANSPORTER',
      'Can I bid on that one? ₦400,000',
      { loadId: 'load-1' },
    );

    expect(result.actions).toHaveLength(1);
    expect(result.actions[0].actionName).toBe('place_bid');
    ...
    mockPrisma.$queryRawUnsafe.mockResolvedValueOnce([
      {
        ...
        action_name: 'place_bid',
        status: 'PENDING_APPROVAL',
        payload: { loadId: 'load-2', amount: 400000 },
        expires_at: new Date(Date.now() + 30 * 60 * 1000),
      },
    ]);

    const prepared = await processAutomationMessage(
      'transporter-1',
      'TRANSPORTER',
      'Place the bid',
      { loadId: 'load-2', amount: 400000 },
    );

    expect(prepared.actions[0].requiresApproval).toBe(true);
    expect(mockPrisma.bid.create).not.toHaveBeenCalled();
    ...
    mockPrisma.$queryRawUnsafe.mockResolvedValueOnce([
      {
        ...
        action_name: 'place_bid',
        status: 'REJECTED',
        payload: { loadId: 'load-3', amount: 400000 },
        expires_at: new Date(Date.now() + 30 * 60 * 1000),
      },
    ]);
