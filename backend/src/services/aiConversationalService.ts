...
function normalizeHistory(history: unknown): HistoryItem[] {
  ...
  return history.filter((item: any) => (item?.role === 'user' || item?.role === 'assistant') && typeof item?.content === 'string').slice(-12).map((item: any) => ({ role: item.role, content: String(item.content).slice(0, 4000) }));
}
...
async function runTool(name: string, args: any, userId: string, role: Role, context: Record<string, unknown>) {
  switch (name) {
    ...
    case 'prepare_bid':
      ...
      return { ...(await processAutomationMessage(userId, role, 'place bid', { loadId: args?.loadId, amount: args?.amount, notes: args?.notes })), instruction: 'The bid is only prepared as PENDING_APPROVAL. The user must approve it before execution.' };
...
async function callOpenAI(userId: string, role: Role, message: string, context: Record<string, unknown>, history: HistoryItem[]) {
  ...
  for (let round = 0; round < 4; round += 1) {
    ...
    if (!response.ok) { if (isQuotaError(response.status, raw)) throw new Error('OPENAI_QUOTA_EXHAUSTED'); throw new Error(`OPENAI_REQUEST_FAILED_${response.status}`); }
    const data = JSON.parse(raw), assistant = data?.choices?.[0]?.message;
    ...
    for (const call of calls) {
      ...
      let output: any;
      try { output = await runTool(call.function?.name, args, userId, role, context); } catch (error: any) { output = { error: error?.message || 'Tool execution failed.' }; }
