...
export default function AdminFinancialOperations() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'HELD': return <span className="bg-indigo-100 text-indigo-800 px-2.5 py-1 rounded-full text-[10px] font-black uppercase flex items-center gap-1 w-fit"><Briefcase size={10} /> In Escrow</span>;
      default: return null;
    }
  };
  ...
  const getTypeIcon = (type: string) => {
    switch(type) {
      case 'ESCROW_DEPOSIT': return <Briefcase size={16} className="text-indigo-500" />;
      case 'PAYOUT': return <ArrowUpRight size={16} className="text-blue-500" />;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
            <Wallet className="text-indigo-600" /> Financial Operations
          </h2>
        ...
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 overflow-x-auto hide-scrollbar flex justify-between gap-4">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
            />
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse animate-fade-in">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {filteredTransactions.map(trx => (
                <tr key={trx.id || trx?.id || Math.random()} className="hover:bg-blue-50 cursor-pointer hover:shadow-sm transition-colors border-b border-slate-100 dark:border-slate-800">
                  <td className="p-4">
