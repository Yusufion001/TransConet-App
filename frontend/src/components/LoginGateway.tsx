...
const benefits = [
  [ShieldCheck, 'Secure', 'Your data is always protected'],
  [Truck, 'Reliable', 'On-time delivery you can trust'],
  [MapPin, 'Track', 'Real-time updates on your shipments'],
  [Headphones, 'Support', "We're here to help 24/7"],
] as const;
...
function Field({ label, icon, value, onChange, type='text', placeholder, right }: any) {
  return <label className="block"><span className="mb-1.5 block text-[12px] font-semibold text-[#4F5B70]">{label}</span><span className="relative block"><span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#8A93A5]">{icon}</span><input required type={type} value={value} onChange={onChange} placeholder={placeholder || label} className="h-14 w-full rounded-xl border border-[#DCE3ED] bg-white pl-11 pr-11 text-[14px] font-medium text-[#172033] outline-none transition placeholder:text-[#9AA3B2] focus:border-[#1457E6] focus:ring-4 focus:ring-[#1457E6]/10"/>{right}</span></label>;
}
...
export default function LoginGateway({ onLoginSuccess }: LoginGatewayProps) {
  ...
  const Google=()=> <Button type="button" disabled={loading} onClick={google} className="h-14 w-full rounded-xl border border-[#DCE3ED] bg-white text-sm font-semibold text-[#172033] shadow-none hover:bg-[#F7F9FC]"><span className="flex items-center justify-center gap-3"><GoogleIcon/>Continue with Google</span></Button>;
