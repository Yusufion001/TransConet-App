...
export default function LoadResultCard({ 
  ...
  return (
    ...
    <motion.div 
      ...
      className="bg-slate-50 dark:bg-slate-800  border border-slate-200 dark:border-slate-700  rounded-2xl p-5 md:p-6 mb-4 shadow-xl  shadow-black/40 hover:border-blue-500/40 transition-all duration-75 overflow-hidden relative"
    >
      <AnimatePresence>
        {status === 'ACCEPTED' && (
          <motion.div 
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-  text-xs font-semibold mt-1 flex items-center gap-2">
            ...
            <span className="text-blue-600 uppercase tracking-wider text-[10px] bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-md font-mono">{truckType}</span>
          </p>
        </div>
      ...
      {/* ROUTE WAYPOINTS */}
      <div className="my-4 border-l-2 border-dashed border-blue-500/30 pl-5 ml-2.5 relative space-y-4">
        {/* Origin dot */}
        <div className="absolute left-[-5px] top-1 w-2.5 h-2.5 rounded-full bg-blue-400 ring-4 ring-blue-400/20" />
        ...
        {/* Destination dot */}
        <div className="absolute left-[-5px] bottom-1 w-2.5 h-2.5 rounded-full bg-blue-500 ring-4 ring-blue-500/20" />
        <div className="space-y-0.5">
      ...
      <Button 
        ...
        className="w-full bg-blue-600 hover:bg-blue-50 cursor-pointer hover:shadow-sm0 disabled:bg-blue-800 disabled:opacity-70 text-white font-bold py-3.5 px-4 rounded-xl text-xs transition-all duration-75 flex items-center justify-center gap-2 cursor-pointer uppercase tracking-wider border border-blue-500/20 shadow-lg  hover:scale-[1.01]"
      >
        {status === 'LOADING' ? (
          <>
