...
function OptionCard({ options, selected, onSelect, label }: { options: { label: string, value: string, icon?: React.ReactNode }[], selected: string, onSelect: (value: string) => void, label: string }) {
  ...
  return (
    <div className="space-y-3 relative">
      <label className="text-sm font-bold text-slate-800 flex items-center justify-center gap-2 text-center">
        {label} <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: ROYAL_BLUE }}></span>
      </label>
      <Button type="button" onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-center gap-3 p-4 bg-white rounded-[20px] transition-colors text-center" style={{ border: `1px solid ${ROYAL_BLUE}` }}>
        {selectedOption ? (
          <div className="flex items-center justify-center gap-3"><span style={{ color: ROYAL_BLUE }}>{selectedOption.icon}</span><span className="text-slate-800 font-medium">{selectedOption.label}</span></div>
        ) : <span className="font-medium text-sm" style={{ color: ROYAL_BLUE }}>Select {label}...</span>}
        <ChevronRight className={`transition-transform ${isOpen ? 'rotate-90' : ''}`} style={{ color: ROYAL_BLUE }} size={20} />
      </Button>
      {isOpen && (
        <div className="absolute z-10 top-[calc(100%+0.5rem)] left-0 w-full bg-white rounded-[20px] shadow-sm p-4 grid grid-cols-2 md:grid-cols-3 gap-3 max-h-64 overflow-y-auto overflow-hidden" style={{ border: `1px solid ${ROYAL_BLUE}` }}>
          {options.map((opt) => (
            <Button type="button" key={opt.value} onClick={() => { onSelect(opt.value); setIsOpen(false); }} className="flex items-center justify-center gap-3 p-3 rounded-xl border text-sm font-medium transition-all bg-white" style={{ borderColor: selected === opt.value ? ROYAL_BLUE : '#B7C6F5', color: selected === opt.value ? ROYAL_BLUE : '#475569' }}>
              <span style={{ color: ROYAL_BLUE }}>{opt.icon}</span>{opt.label}
            </Button>
          ))}
        </div>
      )}
...
export default function CargoDetailsForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="tc-cargo-form bg-white text-slate-600 p-4 md:p-8 space-y-6 max-w-2xl mx-auto min-h-screen">
      ...
      <div className="bg-white p-5 md:p-8 rounded-[20px] space-y-8 shadow-sm overflow-hidden" style={{ border: `1px solid ${ROYAL_BLUE}` }}>
        ...
        <div className="space-y-3"><label className="text-sm font-bold text-slate-800 flex items-center justify-center gap-2 text-center">Cargo Name <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: ROYAL_BLUE }}></span></label><div className="relative"><input name="name" placeholder="Enter cargo name" onChange={handleChange} className="w-full bg-white p-4 pr-12 rounded-[20px] text-slate-800 focus:outline-none transition-colors" required/><Package className="absolute right-4 top-1/2 -translate-y-1/2" style={{ color: ROYAL_BLUE }} size={20}/></div></div>
        ...
        <div className="space-y-3"><label className="text-sm font-bold text-slate-800 flex items-center justify-center gap-2 text-center">Cargo Description <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: ROYAL_BLUE }}></span></label><div className="relative"><textarea name="description" placeholder="Describe your cargo in detail" onChange={handleChange} className="w-full bg-white p-4 pr-12 rounded-[20px] text-slate-800 min-h-[120px] focus:outline-none transition-colors" required/><FileText className="absolute right-4 bottom-4" style={{ color: ROYAL_BLUE }} size={20}/></div></div>
        <div className="space-y-3"><label className="text-sm font-bold text-slate-800 flex items-center justify-center gap-2 text-center">Total Weight ({formData.weightUnit}) <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: ROYAL_BLUE }}></span></label><div className="relative flex items-center"><input type="number" name="weight" placeholder="Enter total weight" onChange={handleChange} className="w-full bg-white p-4 pr-24 rounded-[20px] text-slate-800 focus:outline-none transition-colors" required/><div className="absolute right-4 flex items-center gap-2"><select name="weightUnit" value={formData.weightUnit} onChange={handleChange} className="bg-white outline-none cursor-pointer"><option value="kg">kg</option><option value="tons">tons</option><option value="g">g</option></select><Weight style={{ color: ROYAL_BLUE }} size={18}/></div></div></div>
        ...
        <div className="space-y-3"><Button type="button" onClick={()=>setShowPickup(!showPickup)} className="w-full flex items-center justify-center gap-4 p-4 bg-white rounded-[20px] transition-colors text-center" style={{ border: `1px solid ${ROYAL_BLUE}` }}><div className="flex items-center justify-center gap-4"><div className="p-2 bg-white rounded-xl" style={{ border: `1px solid ${ROYAL_BLUE}` }}><MapPin style={{ color: ROYAL_BLUE }} size={20}/></div><div><h3 className="text-slate-800 font-bold text-sm">Pickup Address</h3></div></div><ChevronRight className={`transition-transform ${showPickup?'rotate-90':''}`} style={{ color: ROYAL_BLUE }} size={20}/></Button>{showPickup&&<div className="p-4 rounded-[20px] space-y-4" style={{ backgroundColor: '#F8FAFF', border: `1px solid ${ROYAL_BLUE}` }}><LocationAutocomplete label="Pickup Location" placeholder="Type pickup city, state or hub (e.g. Apapa Port, Lagos)" value={formData.pickupAddress} onChange={(val)=>setFormData(prev=>({...prev,pickupAddress:val}))} iconColor="text-blue-600"/><input type="date" name="pickupDate" value={formData.pickupDate} onChange={handleChange} className="w-full bg-white p-3 rounded-2xl text-slate-600 focus:outline-none"/></div>}</div>
        <div className="space-y-3"><Button type="button" onClick={()=>setShowDelivery(!showDelivery)} className="w-full flex items-center justify-center gap-4 p-4 bg-white rounded-[20px] transition-colors text-center" style={{ border: `1px solid ${ROYAL_BLUE}` }}><div className="flex items-center justify-center gap-4"><div className="p-2 bg-white rounded-xl" style={{ border: `1px solid ${ROYAL_BLUE}` }}><MapPin style={{ color: ROYAL_BLUE }} size={20}/></div><div><h3 className="text-slate-800 font-bold text-sm">Delivery Address</h3></div></div><ChevronRight className={`transition-transform ${showDelivery?'rotate-90':''}`} style={{ color: ROYAL_BLUE }} size={20}/></Button>{showDelivery&&<div className="p-4 rounded-[20px] space-y-4" style={{ backgroundColor: '#F8FAFF', border: `1px solid ${ROYAL_BLUE}` }}><LocationAutocomplete label="Delivery Location" placeholder="Type delivery destination (e.g. Challenge, Ibadan)" value={formData.deliveryAddress} onChange={(val)=>setFormData(prev=>({...prev,deliveryAddress:val}))} iconColor="text-blue-600"/><input type="date" name="deliveryDate" value={formData.deliveryDate} onChange={handleChange} className="w-full bg-white p-3 rounded-2xl text-slate-600 focus:outline-none"/></div>}</div>
      ...
      <Button type="submit" className="w-full font-bold py-4 rounded-2xl flex items-center justify-center gap-2 transition-all overflow-hidden" style={{ backgroundColor: ROYAL_BLUE, color: '#FFFFFF', border: `1px solid ${ROYAL_BLUE}` }}><Send size={18}/> Post Cargo</Button>
      ...
      {isCalculating&&<div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-white p-6 rounded-[20px] shadow-sm flex flex-col items-center gap-4 overflow-hidden"><div className="w-8 h-8 border-4 rounded-full animate-spin" style={{ borderColor: '#D9E2FF', borderTopColor: ROYAL_BLUE }}></div><p className="font-bold text-slate-800">Posting your cargo...</p></div></div>}
      {calculatedOptions.length > 0&&<div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex flex-col justify-end md:justify-center p-0 md:p-6">
        <div className="bg-white md:rounded-[20px] rounded-t-2xl w-full max-w-3xl mx-auto flex flex-col max-h-[90vh]">
          <div className="p-6 border-b" style={{ borderColor: ROYAL_BLUE }}><h2 className="text-2xl font-bold text-slate-900">Select Transport Option</h2><p className="text-slate-500 mt-1">Based on your cargo details.</p></div>
          <div className="p-6 overflow-y-auto space-y-4">
            {calculatedOptions.map(opt=><button key={opt.id} type="button" aria-pressed={selectedTransportOption?.id===opt.id} onClick={()=>setSelectedTransportOption(opt)} className={`w-full text-left border rounded-xl p-5 transition flex flex-col md:flex-row md:items-center justify-between gap-4 overflow-hidden cursor-pointer focus:outline-none ${selectedTransportOption?.id===opt.id?'bg-white ring-1':'bg-white'}`} style={{ borderColor: selectedTransportOption?.id===opt.id ? ROYAL_BLUE : '#B7C6F5', ...(selectedTransportOption?.id===opt.id ? { boxShadow: `0 0 0 2px rgba(65,105,225,.15)` } : {}) }}>
              <div className="flex gap-4 items-center"><div className="p-4 rounded-xl border" style={{ borderColor: ROYAL_BLUE, color: ROYAL_BLUE }}><Truck size={24}/></div><div><h3 className="font-bold text-slate-900 text-lg leading-tight">{opt.title}</h3><p className="text-slate-500 text-sm">{opt.subtitle}</p></div></div>
              <div className="text-left md:text-right"><p className="uppercase font-black text-slate-400 tracking-wider">Estimated Cost</p><p className="font-black text-2xl" style={{ color: ROYAL_BLUE }}>₦{opt.price.toLocaleString()}</p></div>
            </button>)}
          </div>
          <div className="p-6 border-t bg-white flex justify-between gap-3" style={{ borderColor: ROYAL_BLUE }}>
            <Button type="button" onClick={()=>{setCalculatedOptions([]);setSelectedTransportOption(null);}} className="bg-white font-bold px-6 py-3 rounded-xl transition" style={{ border: `1px solid ${ROYAL_BLUE}`, color: ROYAL_BLUE }}>Cancel</Button>
