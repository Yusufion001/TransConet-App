...
export default function TransConetAIAssistant({ role }: { role: string }) {
  ...
  const send = async (e?: React.FormEvent, overrideMessage?: string, overrideContext?: Record<string, unknown>) => {
    ...
    try {
      const response = await api.post('/ai-automation/assistant', { message: text, history: nextHistory, context: { currentPath: window.location.pathname, ...(referencedLoadId ? { loadId: referencedLoadId } : {}), ...(selectedLoad ? { selectedLoad } : {}), ...(conversationLoads.length ? { marketplaceLoads: conversationLoads } : {}), ...(overrideContext || {}) } });
      ...
      // Keep the complete marketplace result in conversation context. Do not truncate it:
      // follow-up requests such as “highest budget”, “lowest budget”, and “that one” must
      // resolve against every load returned by the real marketplace search, not only the first 20.
  ...
  if (!open) return <button type="button" onClick={() => setOpen(true)} aria-label="Open TransConet AI assistant" className="fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] right-4 z-[210] flex h-12 w-12 items-center justify-center rounded-full bg-slate-900 text-white shadow-xl ring-2 ring-white/80 transition hover:scale-105"><Sparkles size={21} /></button>;
  return <div className="fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] left-3 right-3 z-[210] mx-auto w-auto max-w-md overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
    ...
    <div className="max-h-[min(55vh,32rem)] space-y-3 overflow-y-auto p-4"><div className="rounded-2xl bg-slate-100 p-3 text-sm leading-5 text-slate-800 dark:bg-slate-800 dark:text-slate-100">{reply}</div>{clarifyingQuestion && <div className="rounded-2xl border border-brand-200 bg-brand-50 p-3 text-sm font-semibold leading-5 text-slate-800 dark:border-brand-800 dark:bg-brand-950/30 dark:text-slate-100">{clarifyingQuestion}</div>}
