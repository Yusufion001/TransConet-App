...
export default function AdminContentManagement() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'ARCHIVED': return <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate- px-2 py-0.5 rounded-full text-[10px] font-black uppercase">Archived</span>;
      default: return null;
    }
  };
  ...
  if (isEditorOpen) {
    return (
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm animate-fade-in">
        <div className="flex items-center gap-4 mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
          <Button aria-label="Action" onClick={closeEditor} className="text-slate-400 hover:text-slate-600 dark:text-slate- transition-colors">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-">{editingItem ? `Editing ${editingItem.title}` : 'Drafting a new entry'}</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
                <th className="p-4 text-xs font-bold text-slate-500 dark:text-slate- uppercase tracking-wider">Title</th>
            ...
            <tbody className="divide-y divide-slate-100">
                ...
                <tr key={item.id || item?.id || Math.random()} className="hover:bg-blue-50 cursor-pointer hover:shadow-sm transition-colors border-b border-slate-100 dark:border-slate-800">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-slate-600 dark:text-slate-">{item.type.replace('_', ' ')}</span>
                    </div>
                  </td>
                  <td className="p-4">
              ...
              {filteredContent.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-slate-500 dark:text-slate- bg-white dark:bg-slate-900">
