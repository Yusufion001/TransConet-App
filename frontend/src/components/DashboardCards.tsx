  ...
  >
    ...
    <div className="relative flex min-h-[190px] max-w-xl flex-col justify-center p-5 sm:p-7">
      <div className="flex items-center gap-2 text-brand-700 dark:text-brand-400">
        <Box size={20} />
        <span className="text-[10px] font-bold uppercase tracking-[0.14em]">Operations</span>
      </div>
      <h3 className="mt-3 text-xl font-black tracking-tight text-slate-900 dark:text-white">My Shipments & Fleet</h3>
      <p className="mt-1.5 text-sm leading-relaxed text-slate-600 dark:text-slate-300">Manage active shipments, documents and verified transport providers.</p>
      <div className="mt-4 flex items-center text-sm font-bold text-brand-700 dark:text-brand-400">Open operations <ArrowRight size={16} className="ml-1.5" /></div>
    </div>
  </section>
);
...
export const TrackShipmentCard = ({ engineStatus, waybillInput, setWaybillInput, handleTrackingRequest, isTracking, trackingError }: any) => (
  <section className={`${cardBase} min-h-[190px]`}>
    ...
    <div className="relative p-5 sm:p-7">
      ...
      <p className="mt-1.5 text-sm text-slate-600 dark:text-slate-300">Follow a shipment using its waybill or trip reference.</p>
      <form onSubmit={handleTrackingRequest} className="mt-4 flex flex-col gap-2 sm:flex-row sm:max-w-xl">
        <input
          id="tracking-input"
          type="text"
          placeholder="Waybill or Trip ID"
          className="h-12 min-w-0 flex-1 rounded-xl border border-slate-200 bg-white/95 px-4 text-sm font-mono text-slate-900 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10 dark:border-slate-700 dark:bg-slate-800/95 dark:text-white"
          value={waybillInput}
