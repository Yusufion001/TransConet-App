...
export const FindMarketLoadsCard = ({ onNavigateToNetwork }: { onNavigateToNetwork: () => void }) => (
  <section className="relative overflow-hidden rounded-[20px] border border-slate-200 bg-brand-900 text-white shadow-sm dark:border-brand-500/20">
...
export const MyShipmentsCard = ({ onNavigateToNetwork }: { onNavigateToNetwork: () => void }) => (
  ...
  >
    <img src="/images/transconet-fleet-card.svg" alt="Cargo truck and shipment operations" className="absolute bottom-0 right-[-2%] h-[54%] w-[68%] object-contain object-right-bottom opacity-90" />
    <div className="absolute inset-0 bg-gradient-to-br from-white via-white/96 to-blue-50/35 dark:from-slate-900 dark:via-slate-900/96 dark:to-brand-900/30" />
    <div className="relative flex min-h-[235px] flex-col p-5 sm:min-h-[210px] sm:p-6">
      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-brand-700 dark:bg-brand-900 dark:text-brand-300"><Box size={19} /></div>
...
export const TrackShipmentCard = ({ engineStatus, waybillInput, setWaybillInput, handleTrackingRequest, isTracking, trackingError }: any) => (
  <section className={`${cardBase} min-h-[235px] sm:min-h-[210px]`}>
    <img src="/images/transconet-track-card.svg" alt="Global freight route tracking" className="absolute bottom-0 right-[-2%] h-[43%] w-[72%] object-contain object-right-bottom opacity-85" />
    ...
    <div className="relative flex min-h-[235px] flex-col p-5 sm:min-h-[210px] sm:p-6">
      <div className="flex items-center gap-2">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-400"><Crosshair size={19} /></div>
        <span className="ml-auto rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-400">{engineStatus === 'idle' ? 'GPS ready' : 'Locating'}</span>
      </div>
      ...
      <form onSubmit={handleTrackingRequest} className="relative z-10 mt-auto grid grid-cols-[minmax(0,1fr)_auto] gap-2 pt-4">
        <input id="tracking-input" type="text" placeholder="Waybill or Trip ID" className="h-11 min-w-0 w-full rounded-xl border border-slate-200 bg-white/95 px-3 text-xs font-mono text-slate-900 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10 dark:border-slate-700 dark:bg-slate-800/95 dark:text-white" value={waybillInput} onChange={(e) => setWaybillInput(e.target.value)} />
        <Button type="submit" disabled={isTracking} className="h-11 rounded-xl px-4">{isTracking ? <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> : 'Track'}</Button>
      </form>
...
export const BoostLoadCard = ({ onBoostClick }: { onBoostClick: () => void }) => (
  <section onClick={onBoostClick} className="relative min-h-[180px] overflow-hidden rounded-[20px] border border-brand-800 bg-brand-900 text-white shadow-[0_8px_28px_rgba(11,31,68,0.18)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl">
    ...
    <div className="absolute inset-0 bg-gradient-to-r from-brand-900 via-brand-900/95 to-brand-900/25" />
    <div className="relative flex min-h-[180px] flex-col justify-center p-5 sm:p-7">
      ...
      <Button variant="glass" className="mt-4 h-11 w-fit rounded-xl bg-white px-5 text-slate-900 hover:bg-slate-100"><Truck size={16} className="mr-2" /> Boost Now <Rocket size={15} className="ml-2" /></Button>
    </div>
  </section>
);
