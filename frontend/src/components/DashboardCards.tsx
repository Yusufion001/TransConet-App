...
const cardBase = 'relative overflow-hidden rounded-[24px] bg-white shadow-[0_10px_30px_rgba(15,23,42,0.07)] dark:bg-slate-900';

export const MyShipmentsCard = ({ onNavigateToNetwork }: { onNavigateToNetwork: () => void }) => (
  <section
    onClick={onNavigateToNetwork}
    className={`${cardBase} min-h-[205px] cursor-pointer active:scale-[0.99] transition-transform duration-150`}
  >
    <img src="/images/transconet-fleet-card.svg" alt="Shipment operations" className="absolute bottom-0 right-0 h-[55%] w-[58%] object-contain object-right-bottom opacity-80" />
    ...
    <div className="relative flex min-h-[205px] flex-col p-5 sm:p-6">
...
export const TrackShipmentCard = ({ engineStatus, waybillInput, setWaybillInput, handleTrackingRequest, isTracking, trackingError }: any) => (
  <section className={`${cardBase} min-h-[205px]`}>
    <img src="/images/transconet-track-card.svg" alt="Freight route tracking" className="absolute bottom-0 right-0 h-[42%] w-[62%] object-contain object-right-bottom opacity-65" />
    ...
    <div className="relative flex min-h-[205px] flex-col p-5 sm:p-6">
      ...
      <form onSubmit={handleTrackingRequest} className="relative z-10 mt-auto grid grid-cols-[minmax(0,1fr)_auto] gap-2 pt-4">
        <input id="tracking-input" type="text" placeholder="Waybill or Trip ID" aria-label="Waybill or Trip ID" className="h-12 min-w-0 w-full rounded-2xl border-0 bg-slate-100 px-3.5 text-base font-medium text-slate-900 outline-none transition focus:bg-white focus:ring-2 focus:ring-brand-500/20 dark:bg-slate-800 dark:text-white dark:focus:bg-slate-700" value={waybillInput} onChange={(e) => setWaybillInput(e.target.value)} />
        <Button type="submit" disabled={isTracking} className="h-12 rounded-2xl px-4 text-sm font-extrabold shadow-sm">{isTracking ? <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> : 'Track'}</Button>
      </form>
...
export const BoostLoadCard = ({ onBoostClick }: { onBoostClick: () => void }) => (
  <section onClick={onBoostClick} className="relative min-h-[165px] cursor-pointer overflow-hidden rounded-[24px] bg-[#0B1F44] text-white shadow-[0_12px_32px_rgba(11,31,68,0.16)] active:scale-[0.99] transition-transform duration-150">
    <img src="/images/transconet-boost-card.svg" alt="Premium cargo promotion" className="absolute inset-y-0 right-0 h-full w-[55%] object-cover object-left opacity-60" />
    <div className="absolute inset-y-0 left-0 w-[76%] bg-gradient-to-r from-[#0B1F44] via-[#0B1F44]/96 to-transparent" />
    <div className="relative flex min-h-[165px] flex-col justify-center p-5 sm:p-7">
