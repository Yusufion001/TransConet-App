      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-[20px] p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="font-bold text-slate-700 dark:text-slate-400 text-sm">Generated This Month</h3>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-[20px] p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="font-bold text-slate-700 dark:text-slate-400 text-sm">Active Schedules</h3>
          </div>
          ...
          <table className="w-full text-left border-collapse animate-fade-in">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
                <th className="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Report Name & ID</th>
                <th className="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Type</th>
                <th className="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Format</th>
                <th className="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status</th>
                <th className="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Generated</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredReports.map(report => (
                <tr key={report.id || report?.id || Math.random()} className="hover:bg-brand-50 cursor-pointer hover:shadow-sm transition-colors border-b border-slate-100 dark:border-slate-800">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-1.5 text-slate-700 dark:text-slate-400 font-medium text-xs">
                  ...
                  <td className="p-4">
                    <div className="text-sm text-slate-700 dark:text-slate-400">{report.generatedAt}</div>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button aria-label="Action" className="text-slate-400 hover:text-slate-700 dark:text-slate-400 transition-colors p-2 rounded-lg hover:bg-slate-100 dark:bg-slate-800" title="Options">
                         <Settings size={16} />
                      </Button>
                    </div>
                  </td>
