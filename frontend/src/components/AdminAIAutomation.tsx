...
export default function AdminAIAutomation() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'INACTIVE': return <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-2 py-0.5 rounded-full text-[10px] font-black uppercase flex items-center gap-1 w-fit"><Power size={10} /> Inactive</span>;
      default: return null;
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Manage intelligent workflows, automated load matching, and pricing engines.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-[20px] p-5 shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="font-bold text-slate-700 dark:text-slate-400 text-sm">Hours Saved</h3>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-[20px] p-5 shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="font-bold text-slate-700 dark:text-slate-400 text-sm">Avg Model Accuracy</h3>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-[20px] p-5 shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="font-bold text-slate-700 dark:text-slate-400 text-sm">Total AI Requests</h3>
          </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-[20px] shadow-sm flex-col w-full ${selectedModel ? 'hidden lg:flex lg:w-1/2' : 'flex'}`}>
          <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800">
            <h3 className="font-bold text-slate-800 dark:text-slate-400">Deployed Models & Workflows</h3>
          </div>
            ...
            {models.map(model => (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2">{model.description}</p>
                </div>
        ...
        {selectedModel ? (
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-[20px] shadow-sm flex flex-col w-full lg:w-1/2 animate-fade-in overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-start justify-between">
              <div>
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm">{selectedModel.description}</p>
              </div>
            </div>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
                  <div className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Accuracy Score</div>
                  <div className="text-2xl font-black text-slate-900 dark:text-white">{selectedModel.accuracy}</div>
                </div>
                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
                  <div className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Request Volume</div>
                ...
                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-800 col-span-2">
                  <div className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Business Impact</div>
                  <div className="text-xl font-bold text-brand-600">{selectedModel.impact}</div>
                </div>
              </div>

              <div className="border-t border-slate-100 dark:border-slate-800 pt-6">
                <h4 className="font-bold text-slate-800 dark:text-slate-400 mb-4">Model Controls</h4>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 border border-slate-200 dark:border-slate-700 rounded-xl">
                    <div>
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400">Toggle whether this AI model is actively running in production.</div>
                    </div>
                    <Button 
                  ...
                  <div className="flex gap-2">
                    <Button className="flex-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-400 font-bold px-4 py-2 rounded-xl text-sm transition-colors flex items-center justify-center gap-2">
          ...
          <div className="hidden lg:flex flex-col items-center justify-center w-1/2 border border-dashed border-slate-300 rounded-2xl bg-slate-50 dark:bg-slate-800/50">
            ...
            <h3 className="text-lg font-bold text-slate-700 dark:text-slate-400">Select an AI Model</h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 max-w-sm text-center">Choose a model from the list to view its metrics, configure parameters, or toggle its status.</p>
          </div>
        )}
      </div>
    </div>
