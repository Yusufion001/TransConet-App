    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 shadow-sm">
      <div className="flex justify-between items-center mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs font-bold text-slate-600 dark:text-slate-">Auto-Approve KYC Complete</span>
          <Button 
      ...
      {loading ? (
        <div className="py-8 text-center text-xs text-slate-500 dark:text-slate- animate-pulse">Loading compliance profiles...</div>
      ) : (
        <>
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700 text-xs text-slate-500 dark:text-slate- uppercase tracking-wider">
          ...
          <div className="md:hidden space-y-4">
            {users.map(user => (
              <div key={user.id} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-4 rounded-2xl shadow-sm space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div>
                    <span className="text-slate-500 dark:text-slate- block mb-1 text-[10px] uppercase">Status</span>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-50">
                  <span className="flex items-center gap-1 text-xs text-slate-700 dark:text-slate-">
                    {user.docs === 'Verified' ? <ShieldCheck size={14} className="text-emerald-500"/> : <Zap size={14} className="text-amber-500"/>}
                    {user.docs}
                  </span>
